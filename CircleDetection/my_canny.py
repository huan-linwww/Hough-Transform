...
class Canny:
    ...
    def __init__(self, Guassian_kernal_size, img, HT_high_threshold, HT_low_threshold):
        ...
        self.angle = np.zeros([self.y, self.x])  # 梯度方向矩阵
    ...
    def Get_gradient_img_s(self):
        ...
        for i in range(W - 3):
            for j in range(H - 3):
                dx[i][j] = np.sum(self.img[i:i + 3, j + 3] * Gx)
                dy[i][j] = np.sum(self.img[i:i + 3, j + 3] * Gy)
        self.img, self.angle = cv2.cartToPolar(dx, dy)  # 转变为极坐标系
    ...
    def canny_algorithm(self):
        ...
        :return: Canny 算法的结果
        '''
        self.img = cv2.GaussianBlur(self.img, (self.Guassian_kernal_size, self.Guassian_kernal_size), 0)  # 进行一次gauss滤波
    ...
    def canny_algorithm_s(self):
        '''
        按照顺序和步骤调用以上所有成员函数。
        :return: Canny 算法的结果
        '''
        self.img = cv2.GaussianBlur(self.img, (self.Guassian_kernal_size, self.Guassian_kernal_size), 0)  # 进行一次gauss滤波
        self.Get_gradient_img_s()
        self.img_origin = self.img.copy()
        self.Non_maximum_suppression()
        self.Hysteresis_thresholding()
        return self.img
